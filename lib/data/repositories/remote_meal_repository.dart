...
class RemoteMealRepository implements MealRepository {
  ...
  factory RemoteMealRepository.withSync() {
    _log('Creating RemoteMealRepository, sync enabled: ${SyncConfig.enabled}');
  ...
  Future<Meal> saveMeal(Meal meal) async {
    ...
    if (_syncService != null) {
      ...
      _syncService.syncMeal(
        ...
        _log('Immediate sync ${success ? "succeeded" : "failed (will retry later)"}');
      });
    }
  ...
  Future<void> deleteMeal(int id) async {
    ...
    if (_syncService != null && meal.serverId != null) {
      _log('Deleting from backend: serverId=${meal.serverId}');
      // Note: Backend delete endpoint would go here
      // For now, we just delete locally and the next pull won't have this meal
    }
  }
