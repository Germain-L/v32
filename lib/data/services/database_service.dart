...
class DatabaseService {
  ...
  static Future<void> _onCreate(Database db, int version) async {
    ...
    ''');
    await db
        .execute('CREATE INDEX idx_meal_images_mealId ON meal_images(mealId)');
    await db.execute(
        'CREATE INDEX idx_meal_images_createdAt ON meal_images(createdAt)');
    ...
    await db
        .execute('CREATE INDEX idx_workouts_server_id ON workouts(server_id)');
    await db.execute(
        'CREATE INDEX idx_workouts_pending_sync ON workouts(pending_sync)');
    ...
    ''');
    await db
        .execute('CREATE INDEX idx_body_metrics_date ON body_metrics(date)');
    await db.execute(
        'CREATE INDEX idx_body_metrics_server_id ON body_metrics(server_id)');
    await db.execute(
        'CREATE INDEX idx_body_metrics_pending_sync ON body_metrics(pending_sync)');
    ...
    await db.execute(
        'CREATE INDEX idx_daily_checkins_server_id ON daily_checkins(server_id)');
    await db.execute(
        'CREATE INDEX idx_daily_checkins_pending_sync ON daily_checkins(pending_sync)');
    ...
    ''');
    await db
        .execute('CREATE INDEX idx_screen_times_date ON screen_times(date)');
    await db.execute(
        'CREATE INDEX idx_screen_times_server_id ON screen_times(server_id)');
    await db.execute(
        'CREATE INDEX idx_screen_times_pending_sync ON screen_times(pending_sync)');
    ...
    ''');
    await db.execute(
        'CREATE INDEX idx_screen_time_apps_screen_time_id ON screen_time_apps(screen_time_id)');
    await db.execute(
        'CREATE INDEX idx_screen_time_apps_package_name ON screen_time_apps(package_name)');
    ...
    await db.execute(
        'CREATE INDEX idx_hydrations_server_id ON hydrations(server_id)');
    await db.execute(
        'CREATE INDEX idx_hydrations_pending_sync ON hydrations(pending_sync)');
  }
  ...
  static Future<void> _onUpgrade(
    ...
    if (oldVersion < 5) {
      ...
      ''');
      await db.execute(
          'CREATE INDEX idx_meal_images_mealId ON meal_images(mealId)');
      await db.execute(
          'CREATE INDEX idx_meal_images_createdAt ON meal_images(createdAt)');
    }
    if (oldVersion < 6) {
      ...
      await db.execute(
          'ALTER TABLE meals ADD COLUMN pending_sync INTEGER NOT NULL DEFAULT 0');
      ...
      await db
          .execute('UPDATE meals SET updated_at = date WHERE updated_at = 0');
    }
    if (oldVersion < 7) {
      ...
      await db.execute('CREATE INDEX idx_workouts_type ON workouts(type)');
      await db.execute(
          'CREATE INDEX idx_workouts_server_id ON workouts(server_id)');
      await db.execute(
          'CREATE INDEX idx_workouts_pending_sync ON workouts(pending_sync)');
    ...
    if (oldVersion < 8) {
      await db.execute(
        'ALTER TABLE meals ADD COLUMN remote_image_url TEXT',
      );
      await db.execute(
        'ALTER TABLE meal_images ADD COLUMN remote_url TEXT',
      );
    }
  }
